...
class DeepAndCrossV8(nn.Layer):
    def __init__(self, config):
        ...
        self.senet = SqueezeExcitation(num_fields=self.num_fields * 2, excitation_activation=self.excitation)
    ...
    def forward(self, features, mask):
        ...
        if self.is_Stacked:
            ...
            last_out = last_out.reshape(shape=[-1, self.num_fields * 2, self.sparse_feature_dim])
            se_out = self.senet(last_out)

            logit = self.fc(se_out.reshape(shape=[-1, self.num_fields * 2 * self.sparse_feature_dim]))
            predict = F.sigmoid(logit)
