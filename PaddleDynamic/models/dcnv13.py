...
class DeepAndCrossV11(nn.Layer):
    def __init__(self, config):
        ...
        self.in_dim = self.sparse_feature_dim * (self.num_fields + len(self.pairs) - len(self.multi_values))
        ...
        self.senet = SqueezeExcitation(num_fields=2*self.in_dim//self.sparse_feature_dim, excitation_activation=self.excitation)
    ...
    def forward(self, features, mask):
        ...
        if self.is_Stacked:
            ...
            se_out = self.senet(last_out.reshape(shape=[-1, 2 * self.in_dim // self.sparse_feature_dim, self.sparse_feature_dim]))
            se_out = se_out.reshape(shape=[-1, 2 * self.in_dim])

            logit = self.fc(se_out)
            predict = F.sigmoid(logit)
